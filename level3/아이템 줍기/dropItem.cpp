...
void dfs(vector<vector<int>> &coordinate, vector<vector<bool>> d, int row, int col, int itemX,int itemY, int cnt){
    ...
    for(int i=0;i<4;i++){
        ...
        if(nextRow<0||nextRow>100||nextCol<0||nextCol>100||d[nextRow][nextCol]||coordinate[nextRow][nextCol]==2||!coordinate[nextRow][nextCol]) continue;
        d[nextRow][nextCol] = 1;
...
void draw(vector<int> &rec, vector<vector<int>> &coordinate){
    for(int i= rec[0]; i<=rec[2];i++){
        if(coordinate[rec[1]][i] != 2) coordinate[rec[1]][i] = 1;
        if(coordinate[rec[3]][i] != 2) coordinate[rec[3]][i] = 1;
    }
    for(int i = rec[1]; i<=rec[3]; i++){
        if(coordinate[i][rec[0]] != 2) coordinate[i][rec[0]] = 1;
        if(coordinate[i][rec[2]] != 2) coordinate[i][rec[2]] = 1;
    }
